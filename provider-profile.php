...
<head>
    ...
    <style>
        ...
        /* Sidebar Styles (unused with sidebar removed) */
        .sidebar {
        ...
        /* Main Content */
        .main-content {
            /* sidebar removed, use full width */
            margin-left: 0;
            padding: 1rem 2rem;
            min-height: 100vh;
            transition: none;
        ...
        /* sidebar removed, no collapsed state needed */
...
.schedule-availability {
    ...
    z-index: auto;
        ...
        .mobile-menu-toggle {
            display: none; /* hide toggle since sidebar is gone */
            position: fixed;
            top: 1rem;
            left: 1rem;
            z-index: 1100;
...
<body>
            ...
            <div>
                ...
                <div class="booking-form">
                    ...
                    <?php if (!isLoggedIn()): ?>
                        ...
                        <a href="login.php?next=client/booking.php?provider_id=<?php echo $provider_id; ?>" class="btn btn-primary w-100">
                    ...
                    <?php else: ?>
                        <form method="POST" id="bookingForm" data-working-days="<?php echo htmlspecialchars($schedule_info['working_days'] ?? "1,2,3,4,5"); ?>">
                                ...
                                <select name="service_id" class="form-control" id="serviceSelect" required>
                                    ...
                                    <?php foreach ($services as $service): ?>
                                        <option value="<?php echo $service['id']; ?>"
                                                data-negotiable="<?php echo $service['negotiable'] ? '1' : '0'; ?>"
                                                data-name="<?php echo htmlspecialchars($service['name']); ?>"
                                                data-min-price="<?php echo $service['min_price'] ?? $service['price']; ?>"
                                                data-max-price="<?php echo $service['max_price'] ?? $service['price']; ?>"
                                                data-base-price="<?php echo $service['price']; ?>">
                                            <?php echo htmlspecialchars($service['name']); ?> - RWF <?php echo number_format($service['price'], 0); ?>
                                            <?php if ($service['negotiable']): ?><span class="badge bg-purple ms-1" style="background: linear-gradient(135deg, #667eea, #764ba2);">Negotiable</span><?php endif; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="time" name="preferred_time" class="form-control"
                                               value="<?php echo isset($_POST['preferred_time']) ? htmlspecialchars($_POST['preferred_time']) : ''; ?>">
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="mt-4 pt-4 border-top">
                        ...
                        <div class="d-flex flex-column gap-2">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <span>
                                    <?php 
                                    if ($visibility['show_exact_location']) {
                                        echo htmlspecialchars($provider['location']);
                                        if ($provider['district']) echo ', ' . htmlspecialchars($provider['district']);
                                    } else {
                                        echo htmlspecialchars($provider['district'] ?? 'Location not specified');
                                    }
                                    ?>
                                </span>
                            </div>
    ...
    <script>
        ...
        // Mobile sidebar toggle (disabled when elements missing)
        ...
        document.getElementById('bookingForm')?.addEventListener('submit', function(e) {
            const serviceDescription = document.querySelector('textarea[name="service_description"]').value.trim();
            const preferredDate = document.querySelector('input[name="preferred_date"]');
            
            if (!serviceDescription) {
                e.preventDefault();
                alert('Please provide a service description');
                document.querySelector('textarea[name="service_description"]').focus();
                return false;
            }
        ...
        document.querySelector('input[name="preferred_date"]').addEventListener('change', updateTimeSuggestions);

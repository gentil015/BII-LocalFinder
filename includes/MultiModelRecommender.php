...
class MultiModelRecommender
{
    ...
    /** Number of request retries when the ML API is temporarily unavailable */
    ...
    /** Last error message captured from the ML client */
    private ?string $lastError = null;

    public function __construct(PDO $db, string $apiBase = 'http://localhost:8000', int $timeout = 3, int $maxRetries = 2, int $retryDelayMs = 250)
    {
        $this->db         = $db;
    ...
    public function isApiHealthy(): bool
    {
        ...
        $healthy = isset($response['status']) && in_array($response['status'], ['healthy', 'degraded'], true);
        if (!$healthy && $this->lastError) {
            error_log('[MultiModelRecommender] Health check failed: ' . $this->lastError);
        }
        return $healthy;
    }
    ...
    {
        return $this->curlRequest('POST', $path, json_encode($payload));
    }

    private function curlGet(string $path): ?array
    {
        return $this->curlRequest('GET', $path);
    }

    private function curlRequest(string $method, string $path, ?string $body = null): ?array
    {
        ...
        for ($attempt = 1; $attempt <= $this->maxRetries; $attempt++) {
            ...
            if ($errno) {
                $this->lastError = "cURL error {$errno} for {$method} {$path}";
                error_log('[MultiModelRecommender] ' . $this->lastError);
            } elseif ($responseBody === false) {
                $this->lastError = "Empty response body for {$method} {$path}";
                error_log('[MultiModelRecommender] ' . $this->lastError);
            } else {
                $decoded = json_decode($responseBody, true);
                if (json_last_error() !== JSON_ERROR_NONE) {
                    $this->lastError = 'JSON decode failure: ' . json_last_error_msg() . ' for ' . $method . ' ' . $path . ' response=' . substr($responseBody, 0, 1024);
                    error_log('[MultiModelRecommender] ' . $this->lastError);
                } elseif ($httpCode < 200 || $httpCode >= 300) {
                    $this->lastError = "HTTP {$httpCode} returned for {$method} {$path}: " . trim($responseBody);
                    error_log('[MultiModelRecommender] ' . $this->lastError);
                } else {

            ...
            <?php if ($booking_id > 0): ?>
                <div class="negotiation-shell" id="negotiationShell" style="margin: 0 1rem 0.75rem; padding: 0.9rem 1rem; border: 1px solid rgba(37, 99, 235, 0.16); border-radius: 12px; background: linear-gradient(135deg, rgba(255,255,255,0.97), rgba(239,246,255,0.95)); box-shadow: 0 6px 18px rgba(15, 23, 42, 0.06);">
                    ...
                    <div id="negotiationActionsContainer" style="margin-top:0.75rem; display:flex; gap:0.5rem; flex-wrap:wrap; align-items:center;">
                        ...
                        <button type="button" id="negotiationPrimaryActionBtn" class="btn btn-primary" style="padding:0.65rem 0.95rem; border-radius:8px; border:none; background:var(--accent, #2563eb); color:white; cursor:pointer;">Send counter-offer</button>
...
const negotiationIsProvider = true;
...
function setNegotiationBusy(isBusy) {
    ...
    button.innerHTML = isBusy ? '<span class="me-2"><i class="fas fa-spinner fa-spin"></i></span>Working…' : 'Send counter-offer';
}
...
function renderNegotiationStatus(status) {
    ...
    if (!status || !status.success) {
        ...
        if (primaryButton) primaryButton.textContent = 'Send counter-offer';
    ...
    if (state === 'counter_pending') {
        ...
        if (primaryButton) {
            primaryButton.textContent = 'Send counter-offer';
    ...
    if (state === 'offer_pending') {
        container.innerHTML = '<div style="display:flex; flex-direction:column; gap:0.25rem;"><strong>Offer received</strong><span>Current offer: <strong>RWF ' + Number(data.offered_price || 0).toLocaleString() + '</strong></span><small>You can accept, reject, or send a counter-offer.</small></div>';
        if (primaryButton) {
            primaryButton.textContent = 'Send counter-offer';
            primaryButton.disabled = false;
        }
    ...
    container.innerHTML = '<div style="display:flex; flex-direction:column; gap:0.25rem;"><strong>Start a structured negotiation</strong><span>Share a counter-offer once the client sends their initial price.</span></div>';
    if (primaryButton) {
        primaryButton.textContent = 'Send counter-offer';
        primaryButton.disabled = false;
    }
...
function submitNegotiation() {
    ...
    if (!price || price <= 0) {
        showNegotiationAlert('Enter a valid price before sending the counter-offer.', 'error');
        return;
    }
    ...
    fetch('../api/service_offers.php', {
        ...
        .then(data => {
            if (data.success) {
                showNegotiationAlert('Counter-offer sent successfully.', 'success');
                if (priceInput) priceInput.value = '';
                refreshNegotiationStatus();
            } else {
                showNegotiationAlert(data.message || 'Unable to send the counter-offer.', 'error');
            }
        })
        .catch(() => showNegotiationAlert('Could not submit the counter-offer.', 'error'))
